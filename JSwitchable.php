...
class JSwitchable  extends CWidget
{
    ...
    /**
     * @var string
     * css selector which is the trigger container
     */
    public $container;
    ...
    public function init()
    {
        ...
        if ($this->debug) {
            $this->registerScriptFile("js/jquery.switchable-{$this->jsVersion}.min.js", CClientScript::POS_HEAD);
        } else {
            // no the min file now !
            $this->registerScriptFile("js/jquery.switchable-{$this->jsVersion}.min.js", CClientScript::POS_HEAD);
        }
        ...
        $jsCode .= <<<SETUP
        jQuery('{$this->container}').switchable({$options});
SETUP;
